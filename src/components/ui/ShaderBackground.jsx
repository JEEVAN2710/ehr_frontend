...
void main(void) {
  ...
  for (float i=1.; i<12.; i++) {
    ...
    vec3 color1 = mix(teal, cyan, sin(i * 0.5 + T * 0.3) * 0.5 + 0.5);
    vec3 color2 = mix(purple, violet, cos(i * 0.3 + T * 0.2) * 0.5 + 0.5);
    vec3 dynamicColor = mix(color1, color2, sin(i) * 0.5 + 0.5);
    col+=.00125/d*dynamicColor;
    float b=noise(i+p+bg*1.731);
    col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
    vec3 bgColor = mix(
      vec3(bg*.08, bg*.35, bg*.32),
      vec3(bg*.25, bg*.12, bg*.35),
      d
    );
    col=mix(col,bgColor,d*0.7);
  }
  O=vec4(col,1);
}`;
